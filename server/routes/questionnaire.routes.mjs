...
// POST /api/questionnaire/create-empty - Create a blank questionnaire container (used by Data Quality v2)
router.post('/create-empty', async (req, res) => {
  try {
    ...
    // Idempotent: return existing DQ v2 container if present
    const existing = (questionnaires[projectId] || []).find(q => q?.isDataQualityV2 === true);
    if (existing) {
      return res.json(existing);
    }

    const questionnaireId = `qnr-dqv2-${Date.now()}`;
    const questionnaire = {
      id: questionnaireId,
      name: name || 'Data Quality v2 Dataset',
      questions: [],
      sections: [],
      quotas: [],
      extractedText: '',
    ...
    console.error('Error creating empty questionnaire:', error);
    return res.status(500).json({ error: 'Failed to create empty questionnaire' });
  }
});
    ...
    try {
      ...
      try {
        ...
        res.status(status).json({ 
          error: e?.message || 'No datamap found for this questionnaire', 
