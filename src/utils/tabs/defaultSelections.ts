...
export function getDefaultTableSelectionsForVariable(
  ...
  // Single select grid: all individual statement tables + Mean Summary Table
  ...
  if (isNumericGrid) {
    if (hasPercentTag) {
      // Only Mean Summary Table
      defaultSelections.add(`${variableName}_MeanSummaryTable`);
    } else if (hasNumberTag) {
      // Both Mean and Sum Summary Tables
      defaultSelections.add(`${variableName}_MeanSummaryTable`);
  ...
  // Open end (non-list): frequency distribution table
  if (isOpenEndType) {
    defaultSelections.add(variableName);
  }
  
  // Open end list: nothing (empty set)
