...
import { useTabulationHelpers } from '../hooks/useTabulationHelpers';
...
export default function Tabs({ projects = [], onNavigateToProject, onHeaderChange }: TabsProps) {
  ...
  const [showStatDebug, setShowStatDebug] = useState(false);
  const [statDebugLogs, setStatDebugLogs] = useState<string[]>([]);
  ...
  // Stat debug logging helper
  const appendStatLog = React.useCallback((msg: string, data?: any) => {
    const enabled = (globalThis as any).__TABEXPORT_DEBUG_STATS === true || showStatDebug;
    if (!enabled) return;
    const line = data !== undefined ? `${msg} ${(() => { try { return JSON.stringify(data); } catch { return String(data); } })()}` : msg;
    setStatDebugLogs(prev => {
      const next = [...prev, line];
      // keep last 300 lines
      return next.length > 300 ? next.slice(next.length - 300) : next;
    });
    // Also mirror to console for easy copying
    // eslint-disable-next-line no-console
    console.warn('[STAT DEBUG UI]', msg, data ?? '');
  }, [showStatDebug]);
  ...
  // Helper function to get selected tables for a variable (same table options logic as Tables view)
  const getTablesForVariable = useCallback((variable: Variable): string[] => {
    const tableOptions = getTableOptionsForVariable(variable, questionnaireQuestions, netSummaryTableSelectedCodes);
    const baseKey = getBaseQuestionNumber(variable.name);
    const selectionSet = variableTableSelections[variable.name]
      || variableTableSelections[baseKey]
      || new Set<string>();
    const defaultSelections = getDefaultTableSelectionsForVariable(variable, questionnaireQuestions);
    const selectedIds = selectionSet.size > 0 ? selectionSet : defaultSelections;
    return tableOptions
      .filter(option => selectedIds.has(option.id))
      .map(option => option.id);
  }, [questionnaireQuestions, netSummaryTableSelectedCodes, variableTableSelections]);
  ...
  const { buildTabSpecsWorkbook, buildTablesOnlyWorkbook } = useTabExport({
    fullRawData,
    variableStatsSelections,
    variableSortByFrequency,
    netSummaryTableSelectedCodes,
  ...
  const handleExportTablesToExcel = useCallback(async () => {
    if (!variables.length) {
      alert('No variables available for export.');
      return;
    }
    if (!selectedQuestionnaire && !activeTabPlan) {
      alert('Please select a questionnaire or tab plan.');
      return;
    }
    ...
    try {
      ...
      const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const url = URL.createObjectURL(blob);
      const baseName = activeTabPlan?.name || selectedQuestionnaire?.name || 'TabSpecs';
      const a = document.createElement('a');
      a.href = url;
      a.download = `${baseName.replace(/[^a-zA-Z0-9]/g, '_')}_Tables.xlsx`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      ...
      alert(`Failed to export tables: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setExportingTables(false);
    }
  }, [activeTabPlan, buildTabSpecsWorkbook, selectedQuestionnaire, variables]);
  ...
  const tabSpecsTypeOptions = useMemo(() => {
    ...
    variables.forEach((variable: Variable) => {
      ...
      // Filter out "ID" and "Unknown" types from the dropdown
      if (type && type !== 'ID' && type !== 'Unknown') {
        types.add(type);
      }
    });
            ...
            <div className="bg-white shadow-sm border border-gray-200 rounded-lg overflow-hidden">
              {qnrViewMode === 'tableSpecs' && (
                <TabSpecsView
                  ...
                  onExportTables={handleExportTablesToExcel}
                  exportingTables={exportingTables}
