        ...
        if (isS6Variable && ((globalThis as any).__TABEXPORT_DEBUG_STATS === true || showStatDebug)) {
          ...
          if (isSingleSelectGrid && variable.statements && Object.keys(variable.statements).length > 0) {
            ...
            if (isMeanSummaryTable || isNetSummaryTable) {
              ...
              const netIndex = isNetSummaryTable ? parseInt(netSummaryMatch[1], 10) : -1;

              const netConfigs = isNetSummaryTable ? netSummaryTableSelectedCodes?.[baseName] : null;

              const netConfig = isNetSummaryTable && netConfigs && netIndex >= 0 ? netConfigs[netIndex] : null;

              if (isNetSummaryTable && (!netConfig || !netConfig.codes || netConfig.codes.length === 0)) {
              ...
              const statementData: Array<{
                ...
                cutValues?: Record<string, { base: number; value: number; percentage?: number }>;
              ...
              for (const stmt of statements) {
                ...
                const cutResponseCounts: Record<string, Record<string, number>> = {};

                if (bannerCols.length > 0) {

                  bannerCols.forEach(col => {

                    cutResponseCounts[col.id] = {};

                    responseOptions.forEach(opt => {

                      cutResponseCounts[col.id][opt.code] = 0;
                ...
                variableData.values.forEach((value: any, rowIdx: number) => {
                  ...
                  responseCounts[matchedCode]++;

                  if (bannerCols.length > 0) {

                    const row = fullRawData?.rows?.[rowIdx];
                    ...
                    bannerCols.forEach(col => {
                      ...
                      if (col.matchesRow(row)) {
                        ...
                        cutResponseCounts[col.id][matchedCode!] = (cutResponseCounts[col.id][matchedCode!] || 0) + 1;
                ...
                const cutValues: Record<string, { base: number; value: number; percentage?: number }> = {};
                ...
                if (bannerCols.length > 0) {
                  ...
                  bannerCols.forEach(col => {
                    ...
                    const cutCounts = cutResponseCounts[col.id] || {};

                    const cutBase = Object.values(cutCounts).reduce((a, b) => a + b, 0);

                    let cutValue = 0;

                    let cutPct: number | undefined;
                    ...
                    if (isMeanSummaryTable) {
                      ...
                      responseOptions.forEach(opt => {
                        ...
                        const count = cutCounts[opt.code] || 0;

                        cutSum += codeValue * count;

                        cutCount += count;

                      });

                      cutValue = cutCount > 0 ? cutSum / cutCount : 0;

                    } else if (isNetSummaryTable) {

                      cutValue = countNetRespondents(netConfig.codes, cutCounts);

                      cutPct = cutBase > 0 ? (cutValue / cutBase) * 100 : 0;

                    }

                    cutValues[col.id] = { base: cutBase, value: cutValue, percentage: cutPct };
                ...
                statementData.push({
                  ...
                  cutValues: bannerCols.length > 0 ? cutValues : undefined
              ...
              if (bannerCols.length > 0) {
                ...
                groupStructure.forEach((group) => {
                  ...
                  const groupCell = dataCutsWorksheet.getRow(headerRow1).getCell(groupStartCol);
                  ...
                  if (group.cutCount > 1) {
                    dataCutsWorksheet.mergeCells(headerRow1, groupStartCol, headerRow1, groupStartCol + group.cutCount - 1);
                  }
                  for (let i = 0; i < group.cutCount; i++) {
                    const cutCol = groupStartCol + i;
                    const bannerCol = bannerCols[group.startIdx + i];
                    const cutCell = dataCutsWorksheet.getRow(headerRow2).getCell(cutCol);
                    ...
                    const statCell = dataCutsWorksheet.getRow(headerRow3).getCell(cutCol);
              ...
              if (showBaseRow && statementData.length > 0) {
                ...
                if (bannerCols.length > 0) {
                  let baseCol = 4;
                  bannerCols.forEach(col => {
                    const cutBase = statementData[0].cutValues?.[col.id]?.base ?? 0;
              ...
              statementData.forEach(stmt => {
                ...
                if (bannerCols.length > 0) {
                  ...
                  bannerCols.forEach(bannerCol => {
                    const cutData = stmt.cutValues?.[bannerCol.id];
                    const cutBase = cutData?.base || 0;
                    const cell = valueRow.getCell(col);
                    if (cutBase === 0) {
                      cell.value = '-';
                    } else {
                      cell.value = cutData?.value ?? 0;
                      if (isMeanSummaryTable) {
                        cell.numFmt = '0.00';
                      }
                    }
                ...
                if (isNetSummaryTable && stmt.percentage !== undefined) {
                  ...
                  if (bannerCols.length > 0) {
                    ...
                    bannerCols.forEach(bannerCol => {
                      const cutData = stmt.cutValues?.[bannerCol.id];
                      const cutBase = cutData?.base || 0;
                      const cell = pctRow.getCell(col);
                      if (cutBase === 0) {
                        cell.value = '-';
                      } else {
                        cell.value = (cutData?.percentage ?? 0) / 100;
            ...
            if (statementMatch) {
              ...
              if (bannerCols.length > 0) {
                let baseCol = 4;
                bannerCols.forEach(col => {
                  const cutBase = Object.values(cutResponseCounts[col.id] || {}).reduce((a, b) => a + b, 0);
                  baseRow.getCell(baseCol).value = cutBase;
                  baseRow.getCell(baseCol).alignment = { horizontal: 'center' };
                  baseRow.getCell(baseCol).font = { italic: true, size: 9 };
                  baseRow.getCell(baseCol).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: STATS_GREY } };
              ...
              sortedResponseOptions.forEach(opt => {
                ...
                if (bannerCols.length > 0) {
                  let col = 4;
                  bannerCols.forEach(bannerCol => {
                    const cutCount = cutResponseCounts[bannerCol.id]?.[opt.code] || 0;
                ...
                if (bannerCols.length > 0) {
                  let col = 4;
                  bannerCols.forEach(bannerCol => {
                    const cutBase = Object.values(cutResponseCounts[bannerCol.id] || {}).reduce((a, b) => a + b, 0);
                    const cutCount = cutResponseCounts[bannerCol.id]?.[opt.code] || 0;
                    const cutPct = cutBase > 0 ? (cutCount / cutBase) : 0;
                    const pctCell = pctRow.getCell(col);
                    if (cutBase === 0) {
                      pctCell.value = '-';
                    } else {
                      pctCell.value = cutPct;
          ...
          const numericGridSummaryColumnCode = isNumericGridSummaryTable

            ? (() => {

                const match = tableName.match(/_(c\d+)[^_]*SummaryTable$/i);
          ...
          if (!isNumericGridSummaryTable && !isNetSummaryTable && !isMeanSummaryTableForSSG && !isSumSummaryTableForSSG && !isVerbatimSummary && !isNumericQuestion) {
            ...
            const columnFromVar = variable.name.match(/c\d+/i)?.[0]?.toLowerCase() || null;
            const columnSuffix = (numericGridSummaryColumnCode || columnFromVar) ? `_${numericGridSummaryColumnCode || columnFromVar}` : '';
            if (isMeanSummaryTable) {

              tableTitle = `Table ${tableNumber}: ${baseQuestionNumber}${columnSuffix}: Mean Summary Table`;

            } else if (isSumSummaryTable) {

              tableTitle = `Table ${tableNumber}: ${baseQuestionNumber}${columnSuffix}: Sum Summary Table`;

            } else if (isMeanNoOutliersSummaryTable) {

              tableTitle = `Table ${tableNumber}: ${baseQuestionNumber}${columnSuffix}: Mean (Outliers Removed) Summary Table`;

            } else if (isSumNoOutliersSummaryTable) {

              tableTitle = `Table ${tableNumber}: ${baseQuestionNumber}${columnSuffix}: Sum (Outliers Removed) Summary Table`;
          ...
          if (!isIndividualNumericGridTableForBase && !isOpenEndForBase && !isNumericGridSummaryTable) {
          ...
          if (isNumericGridSummaryTable && variable.statements) {
            ...
            const baseNumber = baseQuestionNumber.replace(/^Q/i, '');
            ...
            if (variable.codes && Object.keys(variable.codes).length > 0) {
              ...
              question.responseOptions.forEach((respOpt, respIdx) => {
                ...
                const raw = typeof respOpt === 'string' ? `c${respIdx + 1}` : (respOpt?.code || `c${respIdx + 1}`);

                columnCodesRaw.push(String(raw));
              ...
              columnCodesRaw.push('c1');

            }

            const normalizeCol = (code: string): string => {
            ...
            const columnCodesNormalized = columnCodesRaw.map(normalizeCol).filter(Boolean);
            ...
            const selectedColumnCode = numericGridSummaryColumnCode ? normalizeCol(numericGridSummaryColumnCode) : null;

            let columnCodes = columnCodesNormalized.length > 0 ? columnCodesNormalized : ['c1'];

            if (selectedColumnCode && columnCodes.includes(selectedColumnCode)) {

              columnCodes = [selectedColumnCode];

            } else if (columnFromVar && columnCodes.includes(columnFromVar)) {

              columnCodes = [columnFromVar];
            ...
            statementEntries.forEach(([stmtCode]) => {
              ...
              columnCodes.forEach(columnCode => {
                ...
                const baseWithQ = `Q${baseNumber}`;

                const cellHeader = `${baseWithQ}${normalizedCode}${columnCode}`;
                ...
                const variations = [
                  ...
                  `${baseNumber}${normalizedCode}${columnCode}`,

                  `${baseQuestionNumber}${normalizedCode}${columnCode}`
            ...
            // Base row (total answering) for numeric grid summary tables
            if (statementEntries.length > 0) {
              const firstStmtCode = statementEntries[0][0];
              const baseData = gridData[firstStmtCode];
              const baseRow = dataCutsWorksheet.getRow(currentRow++);
              const labelCell = baseRow.getCell(2);
              labelCell.value = 'Base (total answering):';
              labelCell.alignment = { horizontal: 'left', vertical: 'middle' };
              ...
              let bc = 3;
              const totalCell = baseRow.getCell(bc++);
              totalCell.value = baseData?.total.base || 0;
              totalCell.alignment = { horizontal: 'center', vertical: 'middle' };
              totalCell.font = { italic: true, size: 9 };
              totalCell.border = { top:{style:'thin'}, bottom:{style:'thin'}, left:{style:'thin'}, right:{style:'thin'} };
              totalCell.fill = { type:'pattern', pattern:'solid', fgColor:{ argb:'FFF2F2F2' } };
              ...
              bannerCols.forEach(col => {
                const cell = baseRow.getCell(bc++);
                cell.value = baseData?.cuts[col.id]?.base || 0;
                cell.alignment = { horizontal: 'center', vertical: 'middle' };
                cell.font = { italic: true, size: 9 };
                cell.border = { top:{style:'thin'}, bottom:{style:'thin'}, left:{style:'thin'}, right:{style:'thin'} };
                cell.fill = { type:'pattern', pattern:'solid', fgColor:{ argb:'FFF2F2F2' } };
            ...
            statementEntries.forEach(([stmtCode, stmtLabel]) => {
              ...
              if (isSumSummaryTable) {
                ...
                // console.log('🟣 Mean row - statement:', stmtLabel, 'total.base:', data.total.base, 'total.mean:', data.total.mean);

                // If total base is 0, show "-" instead of mean

                if (data.total.base === 0) {

                  // console.log('🔴 ZERO BASE DETECTED - Setting mean to dash for:', stmtLabel, 'base:', data.total.base);
