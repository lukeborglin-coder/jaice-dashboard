    ...
    try {
      ...
      for (const variable of variablesToExport) {
        ...
        if (isS6Variable && ((globalThis as any).__TABEXPORT_DEBUG_STATS === true || showStatDebug)) {
          ...
          // CRITICAL EARLY CHECK: Skip base single select grid variable (only render individual statement tables)
          // This must happen HERE before ANY worksheet modifications
          if (isSingleSelectGrid && !isSingleSelectGridIndividualTable && variable.statements && Object.keys(variable.statements).length > 0) {
            console.log('🟠 VERY EARLY SKIP - Base SSG variable (has statements):', tableName);
            tableNumber++;
            continue;
          }

          // CRITICAL EARLY CHECK: For SSG individual tables, check if they have data BEFORE creating headers
          if (isSingleSelectGridIndividualTable) {
            // Quick check: try to find the column header for this statement
            const baseName = variable.name;
            const stmtMatch = tableName.match(/_([^_]+)$/);
            if (stmtMatch) {
              const stmtCode = stmtMatch[1];
              ...
              let stmtColHeader = getColumnHeader(stmtHeader) ||
                                   columnMapping[stmtHeader] ||
                                   columnMapping[stmtHeader.replace(/^Q/, '')];

              // Try additional variations
              if (!stmtColHeader) {
                const variations = [
                  stmtHeader,
                  stmtHeader.replace(/^Q/, ''),
                  `${baseName}${normalizedStmtCode}`
                ];
                for (const v of variations) {
                  if (columnMapping[v]) {
                    stmtColHeader = columnMapping[v];
                    break;
                  }
                  const matchKey = Object.keys(columnMapping).find(k => k.toLowerCase() === v.toLowerCase());
                  if (matchKey) {
                    stmtColHeader = columnMapping[matchKey];
                    break;
                  }
                }
              }

              // If no column header found, skip this table (no data possible)
              if (!stmtColHeader) {
                console.log('🟠 VERY EARLY SKIP - SSG individual table (no column header):', tableName);
                tableNumber++;
                continue;
              }
              ...
              if (fullRawData && fullRawData.rows) {
                for (const row of fullRawData.rows) {
              ...
              // If no data found, skip this table
              if (!hasAnyData) {
                console.log('🟠 VERY EARLY SKIP - SSG individual table (no data in column):', tableName);
                tableNumber++;
                continue;
              }
            }
          ...
          if (!isNumericGridSummaryTable && !isNetSummaryTable && !isMeanSummaryTableForSSG && !isSumSummaryTableForSSG && !isVerbatimSummary && !isSingleSelectGridIndividualTable && !isNumericQuestion) {
            tablePositions.push({ tableNumber, tableName, rowNumber: currentRow, variable });
          ...
          if (!isSingleSelectGridIndividualTableForBase && !isIndividualNumericGridTableForBase && !isOpenEndForBase) {
          ...
          if (isSingleSelectGridIndividualTable) {
            ...
            {
              // Get column header for this statement (do this BEFORE creating title)
              ...
              // Skip empty tables (no base/no data)
              const hasAnyData = responseOptionValues.some(opt => (frequencyMap[opt.code]?.total || 0) > 0);
              // console.log('🔍 SSG Table Check:', tableName, 'totalBase:', totalBase, 'hasAnyData:', hasAnyData, 'responseOptions:', responseOptionValues.length);
              if (totalBase === 0 || !hasAnyData) {
                console.log('🟠 SKIPPING EMPTY SSG TABLE - No data for:', tableName);
                tableNumber++;
                continue;
              }
              ...
              // Table title
              const baseQuestionNumber = getBaseQuestionNumber(variable.name);
              const tableTitle = `Table ${tableNumber}: ${baseQuestionNumber}: ${matchedStmtText}`;
              const titleRow = dataCutsWorksheet.getRow(currentRow++);
              titleRow.getCell(2).value = tableTitle;
              titleRow.getCell(2).font = { bold: true, size: 12 };
              ...
              // Group banner columns by group
              const groupStructure: Array<{ title: string; startIdx: number; cutCount: number }> = [];
              if (bannerCols.length > 0) {
                let currentGroupIdx = bannerCols[0].groupIdx;
                let currentGroupTitle = bannerCols[0].groupTitle || 'Group';
                let currentGroupStartIdx = 0;
                let currentGroupCutCount = 0;

                bannerCols.forEach((col, idx) => {
                  if (col.groupIdx !== currentGroupIdx && idx > 0) {
                    groupStructure.push({
                      title: currentGroupTitle,
                      startIdx: currentGroupStartIdx,
                      cutCount: currentGroupCutCount,
                    });
                    currentGroupIdx = col.groupIdx;
                    currentGroupTitle = col.groupTitle || 'Group';
                    currentGroupStartIdx = idx;
                    currentGroupCutCount = 0;
                  }
                  currentGroupCutCount++;
                });
                // Push the last group
                groupStructure.push({
                  title: currentGroupTitle,
              ...
              groupStructure.forEach((group, groupIdx) => {
                ...
                for (let i = 0; i < group.cutCount; i++) {
                  ...
                  // Stat letter (row 3)
                  const statLetterCell = dataCutsWorksheet.getRow(headerRow3).getCell(cutCol);
                  statLetterCell.value = `(${String.fromCharCode(65 + group.startIdx + i)})`;
                  statLetterCell.alignment = { horizontal: 'center', vertical: 'middle' };
                  statLetterCell.font = { bold: true, color: { argb: 'FFFFFFFF' } };
                  statLetterCell.fill = {
                    type: 'pattern',
                    pattern: 'solid',
                    fgColor: { argb: 'FFD14A2D' }
                  };
                  statLetterCell.border = {
                    top: { style: 'thin' },
                    bottom: { style: 'thin' },
                    left: { style: 'thin' },
                    right: { style: 'thin' }
              ...
              // Add Base (total responding) row AFTER headers
              ...
              responseOptionValues.forEach((respOpt) => {
                ...
                // Total count
                const totalCountCell = countRow.getCell(3);
                // If total base is 0, show "-" instead of count
                totalCountCell.value = totalBase === 0 ? '-' : freqData.total;
                ...
                bannerCols.forEach(bannerCol => {
                  const cutBase = cutBases[bannerCol.id] || 0;
                  // If base size is 0, show "-" instead of count
                  countRow.getCell(col).value = cutBase === 0 ? '-' : (freqData.cuts[bannerCol.id] || 0);
                ...
                netCodes.forEach(net => {
                  if (net.codes && net.codes.length > 0) {
                    ...
                    bannerCols.forEach(bannerCol => {
                      const cutBase = cutBases[bannerCol.id] || 0;
                      // If base size is 0, show "-" instead of count
                      countRow.getCell(col).value = cutBase === 0 ? '-' : netCutCounts[bannerCol.id];
                      countRow.getCell(col).alignment = { horizontal: 'center' };
              ...
              if (statsSelections.mean) {
                ...
                meanRow.getCell(2).font = { bold: false };
